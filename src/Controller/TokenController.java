...
public class TokenController {
	...
	public void updateToken(int toolCardID, int roundID, int gameID, int playerID, int cost, int toolCardIndex) {
		if (this.getAddedTokenAmount(playerID) > cost) {
			...
			for (int i = 0; i < this.getAddedTokenAmount(playerID); i++) {
				cardView.getTokens().getChildren()
						.add(new TokenPane(gameController.getPlayerController().getPlayerColor()));
			}
			...
			Alert notEnoughTokens = new Alert(AlertType.INFORMATION,
					"je hebt niet genoeg betaal stenen voor deze actie", ButtonType.OK);
	...
	public void reloadRemainingDice(int playerID, boolean isOpponent, Color playerColor) {
		WindowPatternView cardView = null;
		if (isOpponent) {
			cardView = windowPatternView;
		} else {
			cardView = gameController.getGamePane().getOwnWindow();
		}
		cardView.getTokens().getChildren().clear();

		for (int i = 0; i < this.getAddedTokenAmount(playerID); i++) {
			cardView.getTokens().getChildren().add(new TokenPane(playerColor));
		}
